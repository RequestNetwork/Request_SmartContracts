...
contract('RequestCore Additional & Subtract Request', function(accounts) {
	...
	it("addAdditional request declined OK - check event log and request status", async function () {
	...
	it("addAdditional request canceled OK - check event log and request status", async function () {
...
/*
	...
	it("new addAdditional _amount+request.amounPaid > amountExpected-request.amountSubtract+request.amountAdditional", async function () {
		...
		assert.equal(r.logs[0].event,"LogRequestAddAdditional","Event LogRequestAddAdditional is missing after addAdditional()");
		assert.equal(r.logs[0].args.requestId,1,"Event LogRequestAddAdditional wrong args requestId");
		assert.equal(r.logs[0].args.amountAdded,arbitraryAmount*2,"Event LogRequestAddAdditional wrong args amountAdded");
